...
class ForecastService:
    ...
    # Valid symptom types
    VALID_SYMPTOMS = {'fever', 'cough', 'gi', 'all', 'total_fever', 'total_cough', 'total_gi'}
        ...
        try:
            ...
            query = self.db.query(ForecastResult).filter(
                and_(
                    ...
                    ForecastResult.prediction_date >= target_forecast_date,  # Changed > to >=
    ...
    def _format_forecast_response(
        ...
        """
        # Group forecasts by symptom - use actual symptom names from data
        grouped = {}
        ...
        for forecast in forecasts:
            ...
            forecast_entry = {
                'date': forecast.prediction_date.isoformat(),  # FIX: Convert to string
        ...
        date_range = {
            'start': min(prediction_dates).isoformat() if prediction_dates else forecast_date.isoformat(),  # FIX: Convert to string
            'end': max(prediction_dates).isoformat() if prediction_dates else (forecast_date + timedelta(days=days)).isoformat()  # FIX: Convert to string
        }
        ...
        return {
            'forecast_metadata': {
                'forecast_date': forecast_date.isoformat(),  # FIX: Convert to string
                'generated_at': generated_at.isoformat() + 'Z' if isinstance(generated_at, datetime) else None,  # FIX: Convert to string
    ...
    def _empty_forecast_response(
        ...
        return {
            'forecast_metadata': {
                'forecast_date': forecast_date.isoformat() if forecast_date else None,  # FIX: Convert to string
                'generated_at': None,
                'prediction_days': days,
    ...
    def get_forecast_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'latest_forecast_date': latest_forecast.isoformat(),  # FIX: Convert to string
                'oldest_forecast_date': oldest_forecast.isoformat(),  # FIX: Convert to string
                'available_models': sorted(models),
                'symptoms_covered': sorted(symptoms),
                'date_coverage': {
                    'earliest_prediction': earliest_prediction.isoformat(),  # FIX: Convert to string
                    'latest_prediction': latest_prediction.isoformat()  # FIX: Convert to string
                }
            }
